...
export function computeRoadStatus(incidents: Incident[]): RoadStatus {
    ...
    if (dangerous.length >= 2 || active.some((i) => i.incident_type === 'crash' && i.severity >= 4))
        return 'caution';
    return 'open';
}
...
export function useRoute(allIncidents: Incident[], routeId: string = 'a17'): UseRouteReturn {
    ...
    const routeIncidents = allIncidents.filter((i) =>
        isOnCorridor(i.latitude, i.longitude, routeId),
    );
